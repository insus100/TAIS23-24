...
protected:
    ...
    struct TreeNode {
        ...
        TreeNode(T const& e, Link i = nullptr, Link d = nullptr,
            int alt = 1, int t = 1) : elem(e), iz(i), dr(d), altura(alt), tam_izq(t) {}
    };
...
public:
    ...
    T const& kesimo(int const& k) const {
        if (this->empty()) {
        ...
        try {
            ...
            throw std::out_of_range("Nodo no encontrado.");
        }
    }

    T const& search(Link h, int k) const {
        if (h == nullptr) {
            throw std::out_of_range("Arbol vacio.");
        }
        else {
            if (h->tam_izq < k) {
                if (h->dr == nullptr) {
                ...
                return search(h->dr, k - h->tam_izq);
            }
            else if (h->tam_izq == k) {
                return h->elem;
            }
